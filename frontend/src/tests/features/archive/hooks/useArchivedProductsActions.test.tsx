...
import { selectAllArchivedProducts, selectLoadingFetchAllArchivedProduct } from '@/store/slices/productSlice.ts'
...
describe('useArchivedProductActions Hook', () => {
  const createWrapper = () => {
  ...
  beforeEach(() => {
    ...
    ;(useAppSelector as jest.Mock).mockImplementation((selector) => {
      if (selector === selectAllArchivedProducts) {
        return [
          { _id: '1', name: 'Product 1', price: 100 },
          { _id: '2', name: 'Product 2', price: 200 },
        ]
      }
      if (selector === selectLoadingFetchAllArchivedProduct) {
        return false
      }
  ...
  describe('Инициализация хука', () => {
    it('должен возвращать правильные начальные значения', () => {
      const { result } = renderHook(() => useArchivedProductActions(), {
  ...
  describe('Управление модальным окном подтверждения', () => {
    it('должен открывать модальное окно для удаления', () => {
      ...
      act(() => {
        result.current.handleConfirmationOpen('product1', 'delete')
      })
    ...
    it('должен открывать модальное окно для восстановления', () => {
      ...
      act(() => {
        result.current.handleConfirmationOpen('product1', 'unarchive')
      })
  ...
  describe('Выполнение действий', () => {
    ...
    it('не должен выполнять действие если productToActionId не установлен', async () => {
      const { result } = renderHook(() => useArchivedProductActions(), {
        wrapper: createWrapper(),
      })
