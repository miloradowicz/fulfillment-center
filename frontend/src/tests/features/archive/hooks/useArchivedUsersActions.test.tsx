...
import { selectAllArchivedUsers, selectUsersLoading } from '@/store/slices/userSlice.ts'
...
describe('useArchivedUsersActions Hook', () => {
  const createWrapper = () => {
  ...
  beforeEach(() => {
    ...
    ;(useAppSelector as jest.Mock).mockImplementation((selector) => {
      if (selector === selectAllArchivedUsers) {
        return [
  ...
  describe('Инициализация хука', () => {
    it('должен возвращать правильные начальные значения', () => {
      const { result } = renderHook(() => useArchivedUsersActions(), {
  ...
  describe('Управление модальным окном подтверждения', () => {
    it('должен открывать модальное окно для удаления', () => {
      ...
      act(() => {
        result.current.handleConfirmationOpen('user1', 'delete')
      })
    ...
    it('должен открывать модальное окно для восстановления', () => {
      ...
      act(() => {
        result.current.handleConfirmationOpen('user1', 'unarchive')
      })
  ...
  describe('Выполнение действий', () => {
    ...
    it('не должен выполнять действие если userToActionId не установлен', async () => {
      const { result } = renderHook(() => useArchivedUsersActions(), {
        wrapper: createWrapper(),
      })
